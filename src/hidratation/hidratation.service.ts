...
export class HidratationService {
    ...
    private async insertConsumedHidratation(id: number, mililiters: number) {
        ...
        const [newConsumed] = await db.insert(hidratationTable).values({
            id_user: id,
            mililiters: mililiters,
            date_consumed: new Date(today.getFullYear(), today.getMonth(), today.getDate()),
        }).returning();
        return newConsumed;
    }
    ...
    async getHidratationConsumed(id: number, date: Date) {
        ...
        const resp = await db
            .select()
            .from(hidratationTable)
            .where(and(eq(hidratationTable.date_consumed, date),
